# :uk: Коннектор Logs API Яндекс.Метрики к ClickHouse :bar_chart:
...
## :running: Быстрый старт  
  ...
  2. Заполните конфиги (`.json` файлы в директории `config/folder`). [Описание конфигов](#config-files-description).  
...
Конфигурация осуществляется при помощи аж 3 конфиг файлов. [Описание конфиг-файлов](#config-files-description).  
